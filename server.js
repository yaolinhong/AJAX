...
var server = http.createServer(function (request, response) {
  ...
  if (path === '/index.html') { //路径
    ...
    response.statusCode = 200
    response.setHeader('Content-Type', 'text/javascript;charset=utf-8')
    response.write(fs.readFileSync('src/js2.js'))
    response.end()
  } 
  else if (path === '/html.html') {
    response.statusCode = 200
    response.setHeader('Content-Type', 'text/html;charset=utf-8')
    response.write(fs.readFileSync('src/html.html'))
    response.end()
  } 
  else if (path === '/XML.xml') {
    response.statusCode = 200
    response.setHeader('Content-Type', 'text/xml;charset=utf-8')
    response.write(fs.readFileSync('src/XML.xml'))
    response.end()
  }   
  else if (path === '/json.json') {
    response.statusCode = 200
    response.setHeader('Content-Type', 'text/json;charset=utf-8')
    response.write(fs.readFileSync('src/json.json'))
    response.end()
  } 
  else {
